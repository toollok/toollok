...
interface FunnelStage {
     id: string;
     name: string;
     conversionRate: number; // Percentage of users surviving FROM the previous step
}

export default function ConversionFunnelSimulator() {
     const [initialTraffic, setInitialTraffic] = useState<number>(10000);
     const [customerValue, setCustomerValue] = useState<number>(50); // LTV or Average Order Value
     const [optimizationBoost, setOptimizationBoost] = useState<number>(5); // +5% conversion boost in comparison mode
     ...
     const addStage = () => {
          ...
          setStages([
               ...stages,
               { id: Math.random().toString(), name: `New Stage ${stages.length + 1}`, conversionRate: 50 }
          ]);
     };
     ...
     // Core Mathematical Engine for Current Funnel
     const funnelMetrics = useMemo(() => {
          ...
          const calculatedStages = stages.map((stage, index) => {
               ...
               return {
                    ...
                    usersSurviving,
                    usersLost,
                    percentOfOriginal: (usersSurviving / initialTraffic) * 100
               };
          });
     ...
     // Core Mathematical Engine for Optimized Scenario Funnel
     ...
     // Global Shortcuts: Cmd/Ctrl + Enter to add stage, Cmd/Ctrl + E to export CSV
          ...
          <div className="w-full max-w-7xl mx-auto flex flex-col gap-6">
               ...
               <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                         ...
                         <div>
                              <div className="flex items-center gap-3">
                                   <h2 className="text-2xl font-bold text-white">Conversion Funnel Drop-off Simulator</h2>
                              ...
                              <p className="text-sm text-gray-400">Map out multi-step journeys to visualize bottlenecks and project revenue leakage.</p>
                         </div>
                    </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    {/* Left Column: Funnel Builder */}
                    <div className="lg:col-span-5 flex flex-col gap-6 sticky top-24">

                         <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl">
                              <h3 className="text-white font-bold text-lg mb-6 flex items-center gap-2 border-b border-gray-800/60 pb-3">
                                   <Target size={18} className="text-blue-400" /> Funnel Parameters
                              </h3>

                              {/* Global Inputs */}
                              <div className="grid grid-cols-2 gap-4 mb-6 bg-gray-950 p-4 rounded-2xl border border-gray-800">
                                   <div>
                                        ...
                                        <input
                                             ...
                                             className="w-full bg-gray-900 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white font-mono outline-none focus:border-blue-500"
                                        />
                                   </div>
                                   <div>
                                        <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5 flex items-center gap-1">
                              ...
                              <div className="mb-6 bg-gray-950 p-4 rounded-2xl border border-gray-800 flex flex-col gap-3">
                                   <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-gray-300 flex items-center gap-1.5">
                                             <Layers size={14} className="text-purple-400" /> Scenario Comparison Mode
                                        </span>
                                        <button
                                             onClick={() => setIsComparisonMode(!isComparisonMode)}
                                             className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors ${isComparisonMode ? 'bg-purple-600' : 'bg-gray-800'}`}
                                        >
                                   ...
                                   {isComparisonMode && (
                                        <div className="pt-3 border-t border-gray-900">
                                             <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase mb-1">
                                                  ...
                                                  <span className="text-purple-400">+{optimizationBoost}%</span>
                                             </div>
                                             <input
                                                  type="range"
                                                  min="1" max="15"
                              ...
                              <div className="space-y-4 relative">
                                   <div className="absolute left-4 top-4 bottom-8 w-px bg-gray-800 z-0"></div>

                                   {stages.map((stage, index) => (
                                        <div key={stage.id} className="relative z-10 flex items-start gap-4">
                                             <div className="w-8 h-8 rounded-full bg-gray-950 border border-gray-700 flex items-center justify-center text-xs font-bold text-gray-400 shrink-0 mt-1 shadow-lg">
                                                  {index + 1}
                                             </div>

                                             <div className="bg-gray-950 border border-gray-800 rounded-2xl p-4 flex-grow group">
                                                  <div className="flex items-center justify-between mb-3">
                                                       <input
                                                            type="text"
                                                            value={stage.name}
                                                            onChange={(e) => updateStage(stage.id, "name", e.target.value)}
                                                            className="bg-transparent text-sm font-bold text-white outline-none focus:border-b focus:border-blue-500 w-full mr-2"
                                                       />
                                                       {index !== 0 && (
                                                            <button
                                                  ...
                                                  {index === 0 ? (
                                                       <span className="text-xs text-emerald-400 font-bold bg-emerald-500/10 px-2 py-1 rounded">100% of Top Traffic</span>
                                                  ) : (
                                                       <div className="flex flex-col gap-2">
                                                            <div className="flex justify-between text-xs font-bold text-gray-500 uppercase">
                                                                 <span>Step Conversion Rate</span>
                                                                 <span className="text-blue-400">{stage.conversionRate}%</span>
                                                            </div>
                                                            <input
                                                                 type="range"
                                                                 min="1" max="100"
                              ...
                              <button
                                   ...
                                   className="w-full mt-6 flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all text-sm border border-gray-700"
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                         ...
                         <div className="bg-[#0c121e] border border-gray-800 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden">
                              <h3 className="text-white font-bold text-lg mb-6 flex items-center gap-2 border-b border-gray-800/60 pb-3">
                                   <Activity size={18} className="text-emerald-400" /> Funnel Performance Dashboard
                              </h3>

                              {/* Hero Metrics */}
                              <div className="grid grid-cols-2 gap-4 mb-8">
                                   <div className="bg-gradient-to-br from-emerald-950/40 to-gray-900 border border-emerald-900/50 rounded-2xl p-5">
                                        <span className="text-[10px] text-emerald-500 font-bold uppercase tracking-wider block mb-1">Total Conversion</span>
                                   ...
                                   <div className="bg-gradient-to-br from-blue-950/40 to-gray-900 border border-blue-900/50 rounded-2xl p-5">
                                        <span className="text-[10px] text-blue-500 font-bold uppercase tracking-wider block mb-1">Projected Revenue</span>
                                        <div className="flex items-baseline gap-2">
                              ...
                              <div className="space-y-3 mb-8">
                                   ...
                                   {funnelMetrics.calculatedStages.map((stage, i) => (
                                        <div key={stage.id} className="relative w-full h-12 bg-gray-900 rounded-lg overflow-hidden border border-gray-800 flex items-center group">
                                             <div
                                                  className="absolute left-0 top-0 bottom-0 bg-blue-600/20 border-r-2 border-blue-500 transition-all duration-500 ease-out"
                                             ...
                                             <div className="relative z-10 flex items-center justify-between w-full px-4">
                                                  ...
                                                  <div className="flex items-center gap-3">
                                                       <span className="text-sm font-mono text-gray-300 bg-gray-950 px-2 py-0.5 rounded border border-gray-800">
                              ...
                              {funnelMetrics.worstDropoff.loss > 0 && (
                                   <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-5 flex items-start gap-4">
                                        <div className="w-10 h-10 bg-rose-500/20 rounded-full flex items-center justify-center text-rose-400 shrink-0">
                                        ...
                                        <div>
                                             ...
                                             <p className="text-sm text-gray-400 leading-relaxed mb-3">
                                                  Your largest drop-off occurs between <strong className="text-white">{funnelMetrics.worstDropoff.fromStage}</strong> and <strong className="text-white">{funnelMetrics.worstDropoff.name}</strong>.
                                             </p>
                                             <div className="flex flex-wrap gap-3">
                                                  <span className="text-xs font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30 px-3 py-1.5 rounded-lg">
                                                       {funnelMetrics.worstDropoff.loss.toLocaleString('en-US')} Users Lost
                                                  </span>
                                                  <span className="text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 px-3 py-1.5 rounded-lg">
