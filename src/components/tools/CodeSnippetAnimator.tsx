...
export default function CodeSnippetAnimator() {
     ...
     // Dev Toggle to simulate User Subscription State
     const [isPremiumUser, setIsPremiumUser] = useState(false);

     // Custom Typewriter Animation Logic
     useEffect(() => {
          if (isPlaying) {
               let currentIndex = 0;
               setDisplayedCode("");
               ...
               const interval = setInterval(() => {
                    ...
                    if (currentIndex === code.length) {
                         ...
                         setTimeout(() => setIsPlaying(false), 1500); // Pause at the end before resetting button
                    }
     ...
     const handleDownload = () => {
          alert("Premium Feature Triggered: Processing 4K MP4 Render...");
     };
     ...
     return (
          <div className="w-full max-w-7xl mx-auto flex flex-col gap-6">
               ...
               <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                         <div className="flex items-center gap-3">
                              <h2 className="text-2xl font-bold text-white">Cinematic Code Snippet Animator</h2>
                              ...
                              <button
                                   ...
                                   className="bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-lg transition-colors border border-gray-700"
                                   title="Click to preview the UI for both Free and Premium users"
                              >
                                   Test Premium UI: {isPremiumUser ? "ON" : "OFF"}
                              </button>
                         </div>
                         <p className="text-sm text-gray-400">Convert code blocks into animated typing videos.</p>
                    </div>
               </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-4 bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl flex flex-col gap-8">

                         {/* Code Input */}
                         <div>
                              <label className="text-sm font-bold text-gray-300 mb-3 flex items-center gap-2">
                                   <Type size={16} className="text-purple-400" /> Source Code
                              </label>
                              <textarea
                                   value={code}
                                   onChange={(e) => setCode(e.target.value)}
                                   className="w-full h-48 bg-[#0c121e] border border-gray-700 rounded-xl p-4 text-sm text-gray-300 font-mono outline-none focus:border-purple-500 resize-none custom-scrollbar"
                                   spellCheck="false"
                         ...
                         <div>
                              <label className="text-sm font-bold text-gray-300 mb-3 flex items-center gap-2">
                                   <Palette size={16} className="text-purple-400" /> Syntax Theme
                              </label>
                              <div className="grid grid-cols-3 gap-3">
                                   {(Object.keys(themes) as Array<keyof typeof themes>).map((t) => (
                                        <button
                                             key={t}
                                             onClick={() => setTheme(t)}
                                             className={`py-2 px-3 rounded-lg text-xs font-bold capitalize border transition-all ${theme === t ? "bg-purple-600/20 border-purple-500 text-purple-400" : "bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700"
                                                  }`}
                                        >
                                             {t}
                                        </button>
                         ...
                         <div>
                              <label className="text-sm font-bold text-gray-300 mb-3 flex items-center gap-2">
                                   <Layout size={16} className="text-purple-400" /> Background Environment
                              </label>
                              <div className="flex flex-wrap gap-3">
                                   {gradients.map((grad) => (
                                        <button
                                             ...
                                             className={`w-12 h-12 rounded-xl bg-gradient-to-br ${grad.class} border-2 transition-transform ${bgGradient === grad.class ? "border-white scale-110 shadow-lg" : "border-transparent hover:scale-105 opacity-70 hover:opacity-100"
                                                  }`}
                                             title={grad.id}
                                        />
                                   ))}
                         ...
                         <div>
                              <div className="flex justify-between items-center mb-3">
                                   <label className="text-sm font-bold text-gray-300">Canvas Padding</label>
                                   <span className="text-xs text-purple-400 bg-purple-500/10 px-2 py-1 rounded font-mono">{padding}px</span>
                              </div>
                              <input
                                   type="range"
                                   min="16" max="96" step="8"
                                   value={padding}
                    ...
                    {/* Live Canvas Preview (Right) */}
                    <div className="lg:col-span-8 flex flex-col gap-6">

                         {/* Action Ribbon (Play & Download) */}
                         <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
                              ...
                              <div className="flex items-center gap-3 w-full sm:w-auto">
                                   <button
                                        onClick={handlePlay}
                                        disabled={isPlaying}
                                        className={`w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${isPlaying
                              ...
                              {/* Dynamic Subscription Button Render */}
                              {isPremiumUser ? (
                                   <button
                                        onClick={handleDownload}
                                        className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white border border-emerald-500/50 px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-emerald-600/20"
                                   >
                                        <Download size={16} /> Download 4K Video
                                   </button>
                              ) : (
                                   <div className="relative group w-full sm:w-auto mt-2 sm:mt-0">
                                        {/* Animated 'Subscribe for Download' Badge */}
                                        ...
                                        <button
                                             ...
                                             className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-700 text-white border border-gray-600 px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg relative overflow-hidden group"
                                        >
                                             <Lock size={14} className="text-gray-400 group-hover:text-white transition-colors" />
                                             Want to download?
                                        </button>
                                   </div>
                              )}
                         ...
                         <div className="relative w-full aspect-video rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center border border-gray-800">
                              <div className={`absolute inset-0 bg-gradient-to-br ${bgGradient} opacity-90 transition-all duration-500`}></div>

                              <div
                                   className={`relative z-10 w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${themes[theme].bg} border border-white/10`}
                              ...
                              >
                                   <div className="h-10 px-4 flex items-center gap-2 bg-black/20 border-b border-white/5">
                                        ...
                                        <div className="flex-grow text-center text-xs font-mono text-gray-500 opacity-60 mr-14">
                                             snippet.js
                                        </div>
                                   </div>
                                   ...
                                   <div className={`p-6 md:p-8 font-mono text-sm md:text-base overflow-hidden ${themes[theme].text}`}>
                                        ...
                                        {isPlaying && (
                                             <span className="inline-block w-2.5 h-5 bg-white ml-1 animate-pulse align-middle"></span>
                                        )}
                                   </div>
                              </div>
                         </div>
