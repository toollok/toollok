...
import {
     Search, Monitor, Smartphone, Share2, Code, Copy, Check,
     AlertTriangle, Globe, Image as ImageIcon, Sparkles, Terminal,
     Star, Gauge, CheckCircle2, XCircle, Info, Tag
} from "lucide-react";
...
export default function SerpPreviewOptimizer() {
     ...
     // Real-time SEO & Organic CTR Health Score (0 - 100)
     const seoScoreData = useMemo(() => {
          let score = 0;
          const checks: { label: string; passed: boolean; pts: number }[] = [];

          // Check 1: Title Length (300px to 580px)
          ...
          // Check 2: Description Length (120 to 160 chars approx)
          ...
          // Check 6: Title contains Power Word / Number
          ...
          <div className="w-full flex flex-col gap-6">
               ...
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                         ...
                         <div>
                              <div className="flex items-center gap-3">
                                   <h2 className="text-2xl font-bold text-white">SERP Preview & Meta Tag Optimizer</h2>
                              ...
                              <p className="text-sm text-gray-400">Simulate search engine results, audit target keywords, test rich snippet features, and boost organic CTR.</p>
                         </div>
                    </div>
               </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-6">
                         <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl flex flex-col gap-5">
                              ...
                              <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2 border-b border-gray-800/80 pb-3">
                                   <Sparkles size={16} className="text-indigo-400" /> Page Metadata Settings
                              </h3>

                              {/* Focus Keyword Input */}
                              <div className="space-y-1.5">
                                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                                        <Tag size={12} className="text-amber-400" /> Focus Target Keyword
                                   </label>
                                   <input
                                        type="text"
                                        value={focusKeyword}
                                        ...
                                        className="w-full bg-gray-950 border border-gray-800 rounded-xl px-3.5 py-2 text-xs text-amber-300 font-bold outline-none focus:border-amber-500/50"
                                   />
                              </div>
                              ...
                              <div className="space-y-1.5">
                                   ...
                                   <input
                                        ...
                                        className="w-full bg-gray-950 border border-gray-800 rounded-xl px-3.5 py-2 text-xs text-gray-200 outline-none focus:border-indigo-500/50"
                                   />
                              </div>
                              ...
                              <div className="space-y-1.5">
                                   <div className="flex items-center justify-between">
                                        ...
                                        <span className={`text-[10px] font-mono font-bold ${titleStatus === 'danger' ? 'text-rose-400' : 'text-emerald-400'}`}>
                                   ...
                                   <input
                                        ...
                                        className={`w-full bg-gray-950 border rounded-xl px-3.5 py-2 text-xs text-gray-200 outline-none transition-colors ${titleStatus === 'danger' ? 'border-rose-500/50 focus:border-rose-500' : 'border-gray-800 focus:border-indigo-500/50'
                                             }`}
                                   />
                                   <div className="w-full h-1 bg-gray-950 rounded-full mt-1 overflow-hidden flex">
                                        <div
                              ...
                              <div className="space-y-1.5">
                                   <div className="flex items-center justify-between">
                                        ...
                                        <span className={`text-[10px] font-mono font-bold ${descStatus === 'danger' ? 'text-rose-400' : 'text-emerald-400'}`}>
                                   ...
                                   <textarea
                                        ...
                                        rows={3}
                                        className={`w-full bg-gray-950 border rounded-xl p-3 text-xs text-gray-200 outline-none resize-none transition-colors ${descStatus === 'danger' ? 'border-rose-500/50 focus:border-rose-500' : 'border-gray-800 focus:border-indigo-500/50'
                                             }`}
                                   />
                                   <div className="w-full h-1 bg-gray-950 rounded-full mt-1 overflow-hidden flex">
                                        <div
                              ...
                              <div className="space-y-1.5 pt-2 border-t border-gray-800/80">
                                   ...
                                   <input
                                        ...
                                        className="w-full bg-gray-950 border border-gray-800 rounded-xl px-3.5 py-2 text-xs text-gray-200 outline-none focus:border-indigo-500/50 font-mono"
                                   />
                              </div>

                              {/* Rich Snippet Customizer Toggles */}
                              <div className="space-y-3 pt-3 border-t border-gray-800/80">
                                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                                        <Star size={12} className="text-amber-400" /> Rich Snippet Simulator
                                   </label>
                                   <div className="grid grid-cols-2 gap-3 text-xs text-gray-300">
                                        <label className="flex items-center gap-2 cursor-pointer bg-gray-950 p-2.5 rounded-xl border border-gray-800">
                                             <input type="checkbox" checked={showRating} onChange={(e) => setShowRating(e.target.checked)} className="rounded text-indigo-500 focus:ring-0" />
                                             <span>Star Rating</span>
                                        </label>
                                        <label className="flex items-center gap-2 cursor-pointer bg-gray-950 p-2.5 rounded-xl border border-gray-800">
                                             <input type="checkbox" checked={showDate} onChange={(e) => setShowDate(e.target.checked)} className="rounded text-indigo-500 focus:ring-0" />
                                   ...
                                   {showRating && (
                                        <div className="grid grid-cols-2 gap-3 pt-1">
                                             <div>
                                                  <label className="text-[9px] font-bold text-gray-500 uppercase block mb-1">Rating (1-5)</label>
                                                  <input type="text" value={ratingVal} onChange={(e) => setRatingVal(e.target.value)} className="w-full bg-gray-950 border border-gray-800 rounded-lg px-2.5 py-1 text-xs text-gray-200 outline-none" />
                                             </div>
                                             <div>
                                                  <label className="text-[9px] font-bold text-gray-500 uppercase block mb-1">Reviews Count</label>
                                                  <input type="text" value={reviewCount} onChange={(e) => setReviewCount(e.target.value)} className="w-full bg-gray-950 border border-gray-800 rounded-lg px-2.5 py-1 text-xs text-gray-200 outline-none" />
                                             </div>
                                        </div>
                                   )}
                              </div>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                         ...
                         <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl flex flex-col gap-4">
                              <div className="flex items-center justify-between border-b border-gray-800/80 pb-3">
                                   <div className="flex items-center gap-2">
                                        ...
                                        <h4 className="text-sm font-bold text-gray-200">SEO Health & CTR Optimization Score</h4>
                                   </div>
                                   <div className="flex items-baseline gap-1 bg-gray-950 px-3 py-1 rounded-xl border border-gray-800">
                                        <span className={`text-xl font-black ${seoScoreData.score >= 80 ? 'text-emerald-400' :
                                                  seoScoreData.score >= 60 ? 'text-amber-400' : 'text-rose-400'
                                             }`}>
                                             {seoScoreData.score}
                                        </span>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                   {seoScoreData.checks.map((chk, index) => (
                                        <div key={index} className="flex items-center justify-between p-2.5 bg-gray-950 rounded-xl border border-gray-800/80 text-xs">
                                             <span className="text-gray-300 font-medium truncate pr-2">{chk.label}</span>
                                             {chk.passed ? (
                         ...
                         <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl flex flex-col gap-4 min-h-[480px]">

                              {/* Workspace Controls Header */}
                              <div className="flex flex-wrap items-center justify-between border-b border-gray-800/80 pb-3 gap-2">
                                   <div className="flex items-center gap-2">
                                        <button onClick={() => setActiveTab("google")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${activeTab === "google" ? "bg-indigo-500/20 text-indigo-400 border border-indigo-500/30" : "bg-gray-950 text-gray-500 border border-gray-800 hover:text-gray-300"}`}>
                                             <Search size={14} /> Google SERP
                                        </button>
                                        <button onClick={() => setActiveTab("social")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${activeTab === "social" ? "bg-cyan-500/20 text-cyan-400 border border-cyan-500/30" : "bg-gray-950 text-gray-500 border border-gray-800 hover:text-gray-300"}`}>
                                             <Share2 size={14} /> Social Cards
                                        </button>
                                        <button onClick={() => setActiveTab("code")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${activeTab === "code" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-gray-950 text-gray-500 border border-gray-800 hover:text-gray-300"}`}>
                                   ...
                                   {activeTab === "google" && (
                                        <div className="flex items-center gap-1 bg-gray-950 p-1 rounded-xl border border-gray-800">
                                             <button onClick={() => setDeviceMode("desktop")} className={`p-1.5 rounded-lg transition-all ${deviceMode === "desktop" ? "bg-gray-800 text-white" : "text-gray-500 hover:text-white"}`} title="Desktop Preview">
                                                  <Monitor size={14} />
                                             </button>
                                             <button onClick={() => setDeviceMode("mobile")} className={`p-1.5 rounded-lg transition-all ${deviceMode === "mobile" ? "bg-gray-800 text-white" : "text-gray-500 hover:text-white"}`} title="Mobile Preview">
                                                  <Smartphone size={14} />
                                             </button>
                                        </div>
                                   )}
                                   ...
                                   {activeTab === "social" && (
                                        <div className="flex items-center gap-1 bg-gray-950 p-1 rounded-xl border border-gray-800 text-[10px] font-bold">
                                             <button onClick={() => setSocialPlatform("twitter")} className={`px-2 py-1 rounded-lg transition-all ${socialPlatform === "twitter" ? "bg-gray-800 text-white" : "text-gray-500 hover:text-white"}`}>X / Twitter</button>
                                             <button onClick={() => setSocialPlatform("facebook")} className={`px-2 py-1 rounded-lg transition-all ${socialPlatform === "facebook" ? "bg-gray-800 text-white" : "text-gray-500 hover:text-white"}`}>Facebook</button>
                                             <button onClick={() => setSocialPlatform("discord")} className={`px-2 py-1 rounded-lg transition-all ${socialPlatform === "discord" ? "bg-gray-800 text-white" : "text-gray-500 hover:text-white"}`}>Discord</button>
                                        </div>
                                   )}
                              </div>

                              {/* PREVIEW DISPLAY CONTAINER */}
                              <div className="flex-grow flex items-center justify-center bg-[#0f1115] rounded-xl border border-gray-800/80 p-6 overflow-hidden min-h-[340px]">

                                   {/* 1. GOOGLE SERP PREVIEW */}
                                   {activeTab === "google" && (
                                        <div className={`transition-all bg-white rounded-lg p-5 font-sans ${deviceMode === "desktop" ? "w-[600px] shadow-sm" : "w-[375px] shadow-lg border border-gray-200"}`}>
                                   ...
                                   {/* 2. MULTI-PLATFORM SOCIAL PREVIEWS */}
                                   {activeTab === "social" && (
                                   ...
                                   {activeTab === "code" && (
                                        <div className="w-full h-full flex flex-col">
                                             <div className="flex items-center justify-between mb-2">
                                                  <span className="text-[10px] font-bold text-gray-400 uppercase flex items-center gap-1.5"><Terminal size={12} /> Production Meta Tags</span>
                                                  <button
                                                       onClick={copyTags}
                                                       className="flex items-center gap-1.5 text-[10px] font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg transition-colors"
                                                  >
                                                       {isCopied ? <Check size={12} /> : <Copy size={12} />}
                                                       {isCopied ? "Copied Tags" : "Copy Head Tags"}
                                                  </button>
                                             </div>
                                             <pre className="bg-gray-950 border border-gray-800 rounded-xl p-4 text-xs font-mono text-cyan-300/90 whitespace-pre-wrap overflow-y-auto flex-grow h-full shadow-inner">
                                                  {metaTags}
                                             </pre>
                                        </div>
                                   )}
