...
export default function TradeJournalAnalyzer() {
     ...
     const [showGraph, setShowGraph] = useState<boolean>(false); // Collapsed by default for clean UX
     ...
     // Interactive Equity Chart Tooltip State
     ...
     // Performance Analytics & Equity Curve Calculations
     const { analytics, equityCurveData } = useMemo(() => {
          const totalTrades = trades.length;
          const sorted = [...trades].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

          let runningEquity = 0;
          const points = [{ index: 0, date: "Start", equity: 0, pnl: 0, symbol: "Initial Capital" }];

          sorted.forEach((t, idx) => {
               runningEquity += t.pnl;
               points.push({ index: idx + 1, date: t.date, equity: runningEquity, pnl: t.pnl, symbol: t.symbol });
          });

          if (totalTrades === 0) {
               return {
                    analytics: { netPnL: 0, winRate: 0, profitFactor: 0, maxDrawdown: 0, winningTrades: 0, losingTrades: 0 },
                    equityCurveData: points
               };
          }
          ...
          return {
               analytics: { netPnL, winRate, profitFactor, maxDrawdown, winningTrades, losingTrades: totalTrades - winningTrades },
               equityCurveData: points
          };
     }, [trades]);
     ...
     const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
          ...
          setHoveredPoint(equityCurveData[index]);
     ...
     const exportCSV = () => {
          ...
          trades.forEach(t => {
               csv += `${t.id},${t.symbol},${t.type},${t.entryPrice},${t.exitPrice},${t.quantity},${t.pnl},${t.setup},${t.date}\n`;
          });
          const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
          const url = URL.createObjectURL(blob);
          const link = document.createElement("a");
          link.setAttribute("href", url);
          link.setAttribute("download", "toollok_trade_journal.csv");
     ...
     useKeyboardShortcuts([
          { key: "enter", ctrlOrCmd: true, action: addTrade },
          ...
          <div className="w-full max-w-7xl mx-auto flex flex-col gap-6">
               ...
               <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                         ...
                         <div>
                              <div className="flex items-center gap-3">
                                   <h2 className="text-2xl font-bold text-white">Institutional Trade Journal & Analyzer</h2>
                              ...
                              <p className="text-sm text-gray-400">Log derivatives and stock trades with real-time expectancy, win-rate, and drawdown metrics.</p>
                         </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                         <button
                              ...
                              className="flex items-center gap-2 bg-gray-900 hover:bg-gray-800 border border-gray-700 text-gray-300 px-3.5 py-2 rounded-xl text-xs font-bold transition-all"
                         >
                              {showGraph ? <EyeOff size={14} className="text-amber-400" /> : <Eye size={14} className="text-emerald-400" />}
                              {showGraph ? "Hide Graph" : "Show Graph"}
                         </button>

                         <button
                              onClick={exportCSV}
                              className="hidden sm:flex items-center gap-2 bg-gray-900 hover:bg-gray-800 border border-gray-700 text-gray-200 px-4 py-2 rounded-xl text-xs font-bold transition-all"
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-lg">
                         ...
                         <span className={`text-2xl font-bold font-mono ${analytics.netPnL >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-lg">
                         ...
                         <span className="text-2xl font-bold font-mono text-rose-400">
                              -₹{analytics.maxDrawdown.toLocaleString('en-IN')}
                         </span>
                    </div>
               </div>

               {/* Collapsible Equity Growth Curve Section */}
               {showGraph && (
                    <div className="bg-[#0c121e] border border-gray-800 rounded-3xl p-6 md:p-8 shadow-xl animate-in fade-in slide-in-from-top-2">
                         <div className="flex items-center justify-between mb-6">
                              <h3 className="text-white font-bold text-base flex items-center gap-2">
                                   <Activity size={18} className="text-emerald-400" /> Cumulative Equity Growth Curve
                              </h3>
                              <span className="text-xs font-mono text-gray-400">Hover graph to inspect values</span>
                         </div>
                         ...
                         <div
                              ...
                              className="relative w-full aspect-[3/1] bg-gray-950 border border-gray-800 rounded-2xl overflow-hidden cursor-crosshair select-none"
                         >
                              <svg
                              ...
                              >
                                   ...
                                   {hoveredPoint && (
                                        <>
                                             <line
                                                  x1={getX(hoveredPoint.index)} y1="0"
                                                  x2={getX(hoveredPoint.index)} y2={chartHeight}
                                             ...
                                             <circle
                                                  ...
                                                  r="6"
                                                  fill={hoveredPoint.equity >= 0 ? "#10b981" : "#f43f5e"}
                                                  stroke="#ffffff"
                                                  strokeWidth="2"
                                             />
                                        </>
                              ...
                              {hoveredPoint && (
                                   <div
                                        className="absolute top-4 bg-gray-900/95 border border-gray-700 backdrop-blur-md rounded-xl px-3 py-2 text-xs shadow-2xl pointer-events-none z-20 flex flex-col gap-1 transform -translate-x-1/2"
                                        style={{ left: `${tooltipLeftPercent}%` }}
                                   >
                                        <div className="flex items-center justify-between gap-4 text-[10px] text-gray-400 font-mono border-b border-gray-800 pb-1">
                                             <span>Date:</span>
                                             <span className="text-white font-bold">{hoveredPoint.date}</span>
                                        </div>
                                        <div className="flex items-center justify-between gap-4 text-[10px] text-gray-400 font-mono">
                                             <span>Symbol:</span>
                                             <span className="text-emerald-400 font-bold">{hoveredPoint.symbol}</span>
                                        </div>
                                        <div className="flex items-center justify-between gap-4 text-[10px] text-gray-400 font-mono">
                                             <span>Trade P&L:</span>
                                        ...
                                        <div className="flex items-center justify-between gap-4 text-[10px] text-gray-400 font-mono pt-1 border-t border-gray-800">
                                             <span>Net Equity:</span>
                                             <span className="text-white font-bold">₹{hoveredPoint.equity.toLocaleString('en-IN')}</span>
                                        </div>
                                   </div>
                              )}
                         </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    {/* Left Column: Log Trade Form */}
                    <div className="lg:col-span-4 bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl flex flex-col gap-4">
                         <h3 className="text-white font-bold text-base border-b border-gray-800/60 pb-3 flex items-center gap-2">
                              <Plus size={16} className="text-emerald-400" /> Log New Trade
                         </h3>
                         ...
                         <div className="grid grid-cols-2 gap-3">
                              <div>
                                   ...
                                   <select
                                        ...
                                        className={`w-full bg-gray-950 border rounded-xl px-3 py-2 text-xs font-bold outline-none ${type === 'Long' ? 'border-emerald-500/50 text-emerald-400' : 'border-rose-500/50 text-rose-400'}`}
                                   >
                         ...
                         <button
                              onClick={addTrade}
                              className="w-full mt-2 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-all text-xs shadow-lg shadow-emerald-600/25"
                    ...
                    <div className="lg:col-span-8 bg-[#0c121e] border border-gray-800 rounded-3xl p-6 shadow-xl overflow-hidden">
                         <h3 className="text-white font-bold text-base mb-4 border-b border-gray-800/60 pb-3 flex items-center justify-between">
                         ...
                         {trades.length === 0 ? (
                              <div className="flex flex-col items-center justify-center h-64 text-center opacity-50">
                                   ...
                                   <p className="text-xs text-gray-400">No trades logged yet. Use the form on the left to add your first trade.</p>
                              </div>
                         ) : (
                              <div className="overflow-x-auto">
                                   <table className="w-full text-left text-xs text-gray-300">
                                        <thead className="bg-gray-950 text-gray-500 font-mono uppercase text-[10px] border-b border-gray-800">
                                             <tr>
                                             ...
                                             {trades.map((t) => (
                                                  <tr key={t.id} className="hover:bg-gray-900/50 transition-colors">
                                                       ...
                                                       <td className="px-4 py-3">
                                                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${t.type === 'Long' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                                                                 {t.type}
                                                            </span>
                                                       </td>
                                                       <td className="px-4 py-3 text-gray-400 font-sans text-xs">{t.setup}</td>
