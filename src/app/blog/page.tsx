          ...
          <div className="w-full min-h-screen bg-[#090d16] text-white selection:bg-blue-500 selection:text-white pb-24">
               ...
               <div className="w-full border-b border-gray-800/60 bg-[#090d16]/80 backdrop-blur-xl sticky top-0 z-50">
                    <div className="max-w-[1440px] mx-auto px-4 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                         <div className="flex items-center gap-6">
                              <Link href="/blog" className="text-xl font-bold bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent">
                                   ToolLok Journal
                              </Link>
                         </div>
               ...
               <main className="max-w-[1440px] mx-auto px-4 lg:px-8 mt-8">
                    ...
                    {!isSearching && featuredPost && (
                         <>
                              {/* News Ticker */}
                              <div className="flex items-center gap-3 bg-gray-900/50 border border-gray-800 rounded-xl p-2 mb-6 overflow-hidden">
                                   ...
                                   <Link href={`/blog/${featuredPost.slug}`} className="text-sm text-gray-300 hover:text-white truncate transition-colors flex items-center gap-2">
                                        {featuredPost.title} <ChevronRight size={14} className="text-gray-500" />
                                   </Link>
                              </div>

                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-16">
                                   {/* LEFT: Large Featured Post */}
                                   <Link href={`/blog/${featuredPost.slug}`} className="group relative block w-full h-[400px] lg:h-[500px] rounded-3xl overflow-hidden border border-gray-800 shadow-xl">
                                   ...
                                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full">
                                        {heroGridPosts.map(post => (
                                             <Link key={post.id} href={`/blog/${post.slug}`} className="group relative block w-full h-[200px] lg:h-[242px] rounded-3xl overflow-hidden border border-gray-800 shadow-lg">
                                                  {/* eslint-disable-next-line @next/next/no-img-element */}
                         ...
                         <div className="lg:col-span-8">
                              ...
                              <div className="flex flex-col gap-6">
                                   {displayedPosts.length > 0 ? displayedPosts.map(post => (
                                        <Link key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col sm:flex-row gap-6 bg-gray-900/30 border border-gray-800 hover:border-gray-600 rounded-3xl p-4 transition-all">
                                             <div className="w-full sm:w-64 h-40 flex-shrink-0 rounded-2xl overflow-hidden">
                                                  {/* eslint-disable-next-line @next/next/no-img-element */}
                                             ...
                                             <div className="flex flex-col justify-center">
                                                  ...
                                                  <h4 className="text-xl font-bold text-gray-100 group-hover:text-blue-400 transition-colors mb-2 line-clamp-2 leading-snug">{post.title}</h4>
